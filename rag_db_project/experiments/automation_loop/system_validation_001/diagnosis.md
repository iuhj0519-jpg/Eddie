...
| 검증 | 결과 |
|---|---|
| 자동화 Unit/Regression Fixture | 21/21 PASS |
| 로컬 RAG 검색 회귀 | 5/5 PASS, 금지 Chunk 0 |
| Systolic Prototype 실제 Compile/Simulation | 종료 코드 0, 99 PASS/1 FAIL, Golden 일치 |
| Optimized 실제 Compile/Simulation | 종료 코드 0, 99 PASS/1 FAIL, Golden 일치 |
| Optimized 실제 Vivado Synthesis 어댑터 | 종료 코드 0, 보고서/DCP 생성; 합성 Error 0, Critical Warning 0, Warning 104 |

위 합성은 약 17분이 걸렸으며, 어댑터를 실제 실행하여 검증한 결과다. 승인된 RTL 최적화 패치를 실행한 결과가 아니다.
원본 증거: artifacts/automation_system_tests/run_001/system_verification.json, unittest.log,
rag_smoke_test.json, initial_failure/, retry/, systolic/.
...
현재 모든 생산 Finding은 미승인이다. run_001/run_002는 보존하고 analysis_001로 최신 근거를 검토한다.
