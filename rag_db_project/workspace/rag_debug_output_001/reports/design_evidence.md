...
| TB 계측 | 기존 선언·DUT 유지. 25개 MAC shadow 누산, 전체 Weight 내용, 797/43/33 Cycle, AXIS/AXI read stall, intr pulse 검사 | 100개 MNIST, 99 PASS/1 FAIL, 기대 label, 10 ns clock, 10 ms timeout 보존 |
...
정정: 원본 TB의 선언이 누락되었다는 초기 진단은 확인 오류였다. run_003은 Agent 계측 패치의 중복 삽입으로 Compile에서 실패했다. 별도 재승인 이후 중복을 제거하여 후속 Run으로 검증하며 실패 이력은 보존한다.
