...
// =====================================================================
// [Testbench] 하이브리드 TPU 검증
// =====================================================================
class HybridTPU_Testbench extends Module {
  ...
  // 간단한 테스트: 모든 입력을 1로 설정
  dut.io.in_input     := VecInit(Seq.fill(16)(1.U(8.W)))
  dut.io.in_weight    := VecInit(Seq.fill(16)(1.U(8.W)))
  dut.io.in_bias      := VecInit(Seq.fill(16)(1.U(32.W)))
  
  dut.io.feed_enable  := cycle < 20.U
  dut.io.load_enable  := cycle === 0.U
  dut.io.swap_weights := cycle === 1.U
  dut.io.bias_load    := cycle === 2.U
  dut.io.accum_en     := cycle > 5.U && cycle < 30.U
  dut.io.clear_all    := cycle === 0.U
  
  dut.io.act_mode_en  := true.B
  dut.io.norm_mode_sel := 2.U  // Softmax 모드
  
  when(cycle === 50.U) {
    printf("[하이브리드 TPU] 최종 출력: ")
    for (i <- 0 until 16) {
      printf("%d ", dut.io.final_post_out(i))
    }
    printf("\n")
  }
}
...
object HybridTPU_Main extends App {
  println("🚀 Hybrid TPU Accelerator SystemVerilog 생성 중...")
  ...
  println("✅ Hybrid TPU 생성 완료!")
  println("   최고 장점 통합:")
  println("   ✓ TPU_revised: 간단한 파이프라인 + Valid 토큰 동기화")
  println("   ✓ MatMul: 온라인 Softmax (Milakov) + 안정적 수치 처리")
  println("   ✓ 새로운 모드_sel=2: Softmax 전용 온라인 처리")
}
